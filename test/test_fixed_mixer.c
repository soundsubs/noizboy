...
/* Verifies the fixed 3-source mixer restructuring, per explicit
 * request: always exactly 2 noise + 1 Karplus source, only mix level
 * (0-100%) randomized, and that randomization happens at the RECIPE
 * level (fixed until the user globally re-randomizes), not re-rolled
 * per note. */

int main(void) {
    int all_ok = 1;

    /* Structure check: always 3 layers, always layers 0/1 = noise, layer 2 = karplus */
    for (unsigned int i = 1; i < 200; i++) {
        NoiseboyEngine e;
        noiseboy_engine_init(&e, 48000.0, i * 7919u);
        if (e.numRecipeLayers != 3) { printf("FAILED: numRecipeLayers should always be 3 (seed %u, got %d)\n", i, e.numRecipeLayers); all_ok = 0; }
    ...
    printf("Mix level range across 500 seeds x 3 layers: %.4f to %.4f (expect close to 0.0-1.0)\n", minMix, maxMix);
       ...
       until global re-randomize, not re-rolled per note). */
    {
        ...
        double mix0 = e.recipe[0].mixLevel01, mix1 = e.recipe[1].mixLevel01, mix2 = e.recipe[2].mixLevel01;
        for (int n = 0; n < 20; n++) {
        ...
        int stable = (e.recipe[0].mixLevel01 == mix0 && e.recipe[1].mixLevel01 == mix1 && e.recipe[2].mixLevel01 == mix2);
       ...
       randomize path still works). */
    {
        ...
        double before0 = e.recipe[0].mixLevel01, before1 = e.recipe[1].mixLevel01, before2 = e.recipe[2].mixLevel01;
        noiseboy_randomize_recipe(&e);
        int changed = (e.recipe[0].mixLevel01 != before0 || e.recipe[1].mixLevel01 != before1 || e.recipe[2].mixLevel01 != before2);
        ...
        if (e.numRecipeLayers != 3 || e.recipe[0].type != LAYER_FILTERED_NOISE ||
    ...
    /* Mute check: a source with mixLevel01=0 should contribute nothing audible */
    {
        ...
        /* verify voice's layer 0/1 outputs are being scaled to 0 -- indirect check via full mute of all sources */
        NoiseboyEngine e2;
